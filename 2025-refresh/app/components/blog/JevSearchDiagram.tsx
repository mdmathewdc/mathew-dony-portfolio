...
export function JevSearchDiagram() {
  ...
  useEffect(() => {
    ...
    import("@joint/core").then((joint) => {
      ...
      // Node colors
      const colors = {
        input: { fill: "#dc2626", stroke: "#ef4444" }, // Red
        model: { fill: "#9333ea", stroke: "#a855f7" }, // Purple
        question: { fill: "#2563eb", stroke: "#3b82f6" }, // Blue
        code: { fill: "#16a34a", stroke: "#22c55e" }, // Green
      };
      ...
      const query = createNode(40, 175, "Query + Catalog", colors.input);
      const jev = createNode(240, 175, "Jev", colors.model);
      const movieChecks = createNode(480, 90, "100 × Movie Match", colors.question);
      const exists = createNode(480, 260, "Exists?", colors.question);
      const rank = createNode(700, 175, "Filter & Rank", colors.code);

      // Create links
      const links = [
        createLink(query, jev),
        createLink(jev, movieChecks),
        createLink(jev, exists),
        createLink(movieChecks, rank),
        createLink(exists, rank),
      ];
